...
FinCredit Copilot v1.2 只接受 `demo-header` 与 `oidc` 两个显式身份提供方。拼写错误或未知 Provider 会返回 `503`，不会退回演示用户表。OIDC 模式要求标准 `Authorization: Bearer <token>`，根据 JWT Header 的 `kid` 从企业 JWKS 选择公钥，并同时校验算法白名单、签发方、受众、签名、有效期和必需声明。
