...
FinCredit Copilot v0.3 只接受 `demo-header` 与 `oidc` 两个显式身份提供方。拼写错误或未知 Provider 会返回 `503`，不会退回演示用户表。OIDC 模式要求标准 `Authorization: Bearer <token>`；当前 JWKS 验签仍是生产扩展点，因此未配置时保持不可用。

设置 `FINCREDIT_ENVIRONMENT=production` 时，`GET /ready` 会拒绝 `demo-header`。风险经理与审批人只能访问申请创建人所属组织的数据，合规管理员保留全局审计权限；客户经理只能访问本人创建的申请。
