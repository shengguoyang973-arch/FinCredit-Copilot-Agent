...
FinCredit Copilot v1.3 将“看到 Prompt 分群指标”升级为可审计的人机协同处置闭环。它固化的是无客户信息的聚合观察快照，并要求另一名合规管理员确认或驳回复盘建议；已确认的排查或受控回滚建议可进一步形成有人负责的处置作业单。该机制不自动激活/回滚 Prompt、不修改规则、不调用模型，也不作出任何授信决定。
