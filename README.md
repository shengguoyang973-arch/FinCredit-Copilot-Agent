...
已确认复盘的人工处置作业单、负责人、时限、状态与结论证据见 [docs/prompt-remediation.md](docs/prompt-remediation.md)。
...
可观测性接口为 `GET /v1/observability/agent-metrics` 与 `GET /v1/observability/prompt-performance`，需要 `X-User-Id: compliance_001`。前者聚合 Agent 运行次数、降级率、平均耗时、最大耗时、Provider 分布、任务分布、工具调用次数和最近运行记录；后者按冻结 Prompt 版本汇总运行量、人工反馈、最终人工审批工作流结果、复盘状态和最新处置作业单摘要。工作台内置“Agent 运行观测”面板，切换到“周合规管理员”后可直接刷新查看；生成预审报告、发起 AI 业务问答或完成人工审批后，指标会随之更新。所有 HTTP 响应都会带 `X-Request-Id`，请求日志使用 JSON 结构输出。
