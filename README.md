...
面向小微企业流动资金贷款的授信尽调与审批协同 Agent MVP。系统只提供预审建议和报告草稿，绝不自动作出授信决定。
...
`release_gate.py` 在质量门禁之上增加 Prompt/Provider 评测门槛，检查准确率、证据引用率和人工审批边界；`GET /ready` 用于容器就绪探针。
...
Agent 默认使用 `FINCREDIT_AGENT_PROVIDER=deterministic-local`，不会调用外部模型。配置 `FINCREDIT_AGENT_PROVIDER=openai-responses` 且提供 `OPENAI_API_KEY` 后，会通过 OpenAI Provider 生成报告评述和实时业务问答；配置 `FINCREDIT_AGENT_PROVIDER=deepseek-chat` 且提供 `DEEPSEEK_API_KEY` 后，会通过 DeepSeek OpenAI 兼容接口生成报告评述和实时业务问答。如果密钥缺失、调用失败或输出校验失败，系统会自动降级到本地确定性 Provider，并在 Agent 输出中写明降级原因。

每次生成预审报告都会保存一条 Agent 运行记录，可通过 `GET /v1/applications/{application_id}/agent-runs` 查看。运行记录只保存结构化输入摘要、政策证据编号、材料完整性状态和 Agent 输出，不保存原始上传文件正文。
